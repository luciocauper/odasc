...
const getColor = (casos: number) => {
    return casos > 10000 ? '#7a0e26' : // Vinho Muito Escuro
        casos > 7000 ? '#9F4456' : // Vinho
            casos > 5000 ? '#C64C5E' : // Vermelho Escuro
                casos > 3000 ? '#D15360' : // Vermelho
                    casos > 2000 ? '#EA5F61' : // Laranja Escuro
                        casos > 1000 ? '#F58C71' : // Laranja
...
export default function MapComponent() {
    ...
    const initMap = useCallback(async () => {
        ...
        try {
            ...
            const featuresFiltradas = data.features.filter((f: any) =>
                dadosCovid[f.properties.codarea]
            );

            const geoJsonLayer = L.geoJSON({ type: 'FeatureCollection', features: featuresFiltradas } as any, {
                style: (feature) => {
                ...
                onEachFeature: (feature, layer) => {
                    ...
                    if (layer instanceof L.Polygon) {
                        ...
                        L.marker(center, { icon: labelIcon, interactive: false }).addTo(mapInstanceRef.current!);
                    }
    ...
    return (
        <div className="flex items-center justify-center min-h-screen bg-slate-100 p-4 md:p-8">
            <div className="w-full max-w-5xl bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden flex flex-col h-[80vh]">
                ...
                <div className="relative flex-1 w-full bg-slate-50">
                    ...
                    <div className="absolute top-4 right-4 z-[400] flex items-center gap-2 bg-white/90 backdrop-blur-sm px-3 py-2 rounded-lg border border-slate-200 shadow-sm">
                        <div className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-[#FCA580]"></span><span className="text-[10px] text-slate-600 font-medium">- Casos</span></div>
                        <div className="w-8 h-[2px] bg-gradient-to-r from-[#FCA580] to-[#7a0e26]"></div>
                        <div className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-[#7a0e26]"></span><span className="text-[10px] text-slate-600 font-medium">+ Casos</span></div>
                    </div>
                </div>
            </div>
        </div>
